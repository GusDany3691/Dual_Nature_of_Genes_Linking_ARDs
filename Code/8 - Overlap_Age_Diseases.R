...
AgeHumGen = readRDS("C:/Users/Usuario/Desktop/Nature/Data/Generated/HAGR/GenAgeHum_Genes.rds")
AgeModGen = readRDS("C:/Users/Usuario/Desktop/Nature/Data/Generated/HAGR/GenAgeMod_Genes.rds")

AgeHumFrm = data.frame(Gen=AgeHumGen, Com="GenAge_Hum")
AgeModFrm = data.frame(Gen=AgeModGen, Com="GenAge_Mod")

GenComFrm = GenPhnComFrm[,c('Gen','Com')] %>% rbind(AgeHumFrm) %>% rbind(AgeModFrm) %>% unique()


# Convert the data frame to wider format
GenComFrm$value <- 1
wider_df <- GenComFrm %>% pivot_wider(names_from = Com, values_from = value, values_fill = 0)
wider_dt = wider_df %>% as.data.frame()
row.names(wider_dt) = wider_dt$Gen
wider_dt$Gen = NULL


NamArr = GenComFrm$Com %>% table() %>% sort() %>% rev() %>% names()
...
NodOvlBinFrm = NodOvlBinFrm %>% filter(Sum>0 & Mrg%in%MrgFlt)  
...
# MAX
MaxNodOvlBinFrm = NodOvlBinFrm %>% filter(Sum == MaxSum)
...
CmpSrtFcn = function(EndArr, ComArr){
  DifNam = setdiff(ComArr,names(EndArr)) 
  if(length(DifNam)>0){
...
Arr2FrmFcn = function(CrdArr, ComArr, Nam){
  tCrdFrm = CrdArr[ComArr] %>% data.frame() 

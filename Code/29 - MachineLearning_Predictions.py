...
GrpSrcArr = ['EndPin','EndC95','EndC90','EndKgo']
Typ = 'UadPinNgb'


# Initialize EndAlgFrm as an empty DataFrame with the specified columns
EndAlgFrm = pd.DataFrame(columns=['Alg', 'Auc', 'Gmn', 'Sns', 'Spc', 'Grp'])


DatArr =  ['GenPhnCls_PrxFrm',
           'GenComCls_PrxFrm',
           'GenPhnAvg_PrxFrm','GenComAvg_PrxFrm',
           'GenPhnNgb_PrxFrm','GenComNgb_PrxFrm']
...
for GrpSrc in GrpSrcArr:
    ...
    for Dat in DatArr:
        ...
        PathModel =  'C:/Users/Usuario/Desktop/Nature/Data/Generated/'+GrpSrc+'/Mod/'
        PathVarimp = 'C:/Users/Usuario/Desktop/Nature/Data/Generated/'+GrpSrc+'/Imp/'

        oDataset = pd.read_csv('C:/Users/Usuario/Desktop/Nature/Data/Generated/'+GrpSrc+'/Dat/'+Typ+'/'+Dat+'.csv')
        ...
        for k in range(0, OuterSplits):
            ...
            if k == (OuterSplits-1):
                ...
                AlgFrm.NewRow = pd.Series([Dat,
                                       int(100*round(AverageAucTest,2)), 
                                       int(100*round(AverageGmeanTest,2)),
                                       int(100*round(AverageSensitivityTest,2)),
                                       int(100*round(AverageSpecificityTest,2))], index=AlgFrm.columns)

                AlgFrm = pd.concat([AlgFrm, pd.DataFrame([AlgFrm.NewRow])], ignore_index=True)

                ModelSavingText = PathModel + Typ + '/' + Df_Nam + Cntr + Classifier + Cntr + Method + tOuterSplits + CvOuterMethod + tInnerSplits + CvInnerMethod + Cntr + 'a' + str(int(100*round(AverageAucTest,2))) + 'g' + str(int(100*round(AverageGmeanTest,2))) + 'sn' + str(int(100*round(AverageSensitivityTest,2))) + 'sp' + str(int(100*round(AverageSpecificityTest,2))) + '.py'
                ...
                # SAVING FRAME
                FrameSavingText = ModelSavingText.replace("Mod", "Pre")
                FrameSavingText = FrameSavingText.replace(".py", ".csv")
...
EndAlgFrm.to_csv('C:/Users/Usuario/Desktop/Nature/Data/Generated/Performances/'+Typ+'/ML_AlgFrm.csv')

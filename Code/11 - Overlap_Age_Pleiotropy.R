...
ComPltFrm = GenPhnComFrm %>% 
  ...
  as.data.frame() %>% 
  arrange(desc(Freq))
colnames(ComPltFrm) = c('Gen','Frq')


# Outer pleitropy
GenPl6 = ComPltFrm %>% filter(Frq==6) %>% pull(Gen) %>% as.character()
GenPl5 = ComPltFrm %>% filter(Frq==5) %>% pull(Gen) %>% as.character()
GenPl4 = ComPltFrm %>% filter(Frq==4) %>% pull(Gen) %>% as.character()
GenPl3 = ComPltFrm %>% filter(Frq==3) %>% pull(Gen) %>% as.character()
GenPl2 = ComPltFrm %>% filter(Frq==2) %>% pull(Gen) %>% as.character()
GenPl1 = ComPltFrm %>% filter(Frq==1) %>% pull(Gen) %>% as.character()

RowGen = c(GenAgeHumArr, GenAgeModArr, GenPl1, GenPl2, GenPl3, GenPl4, GenPl5, GenPl6) %>% unique()
...
UpsFrm = 
  data.frame(
             ...
             'ARC-Pleitropy_1' = RowGen %in% GenPl1,
             'ARC-Pleitropy_2' = RowGen %in% GenPl2,
             'ARC-Pleitropy_3' = RowGen %in% GenPl3,
             'ARC-Pleitropy_4' = RowGen %in% GenPl4,
             'ARC-Pleitropy_5' = RowGen %in% GenPl5,
             'ARC-Pleitropy_6' = RowGen %in% GenPl6#,
  ) * 1
...
UpsPltFcn = function(OrgUpsFrm){
  ...
  # PRINT UPSET
  TtlTxt = "ARCs' pleitropies and GenAge's PPI-based neighbourhood"
  upset(UpsFrm ,
        keep.order = T,
        number.angles = 45,
        sets = PhnArr, 

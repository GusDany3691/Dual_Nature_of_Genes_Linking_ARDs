...
WidComComLst = readRDS("C:/Users/Usuario/Desktop/Nature/Data/Generated/Coexpression/WidComComLst.rds") 
ExtComMenFrm = readRDS("C:/Users/Usuario/Desktop/Nature/Data/Generated/Coexpression/ExtComMenFrm.rds") 
...
ComComScrFrm = readRDS("C:/Users/Usuario/Desktop/Nature/Data/Generated/Coexpression/ComComScrFrm.rds") %>%
...
# ACTUAL PLOT
BoxComScrFrm$Lbl = "Self-Coexpression in genes associated with ARCs and GenAge"
...
p = p + geom_text(data = counts, aes(x = Var1, y = -0.2, label = FreqComma), size=3)
...
write.table(BoxComScrFrm,'C:/Users/Usuario/Desktop/Nature/Data/Generated/Coexpression/BoxComScrFrm.csv', row.names = FALSE, sep=",")
...
# ORIGINAL
PltFrm = WidComComLst$ExtInn$Mix %>%
  filter(InnCom %in% ComPltAgeArr) %>% select(c("InnCom", ComPltAgeArr))

TxtFrm = WidComComLst$ExtInn$Mix %>%
  filter(InnCom %in% ComPltAgeArr) %>% select(c("InnCom", ComPltAgeArr))
...
CatVecRow = rep("Gen", length(RowNam))
CatVecRow[RowNam %in% c("GenAge", "ModAge")] = "Hgr"
CatVecRow[grepl("Plt",RowNam)] = "Plt"

CatVecCol = rep("Gen", length(ColNam))
CatVecCol[ColNam %in% c("CelAge", "GenAge", "GenDR", "ModAge")] = "Hgr"
CatVecCol[grepl("Plt",ColNam)] = "Plt"

#HgrVec = ifelse(RowNam %in% c("CelAge", "GenAge", "GenDR"))
...
FntSiz = 6
TxtDeg = 60
Htm = Heatmap(as.matrix(PltFrm), name = "Mean Coexpression", #top_annotation = TopAnn, #km = 5,
